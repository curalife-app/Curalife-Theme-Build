...
class QuizCore {
  ...
  renderDatePart(question, response) {
    ...
    const partsHTML = parts.map((part) => {
      ...
      const options = this._getDatePartOptions(part).map((option) => {
  ...
  _updateResponse(questionId, answer, stepId) {
    ...
    this.container.dispatchEvent(
      new CustomEvent("quizAnswer", {
        detail: { questionId, answer, stepId }
      })
    );
  }
  ...
  _attachCheckboxListeners(question) {
    ...
    const getSelectedValues = function() {
      return Array.from(checkboxInputs).filter(function(input) {
  ...
  _attachFormCheckboxListeners(question) {
    ...
    checkboxes.forEach(function(checkbox) {
      checkbox.addEventListener("change", function() {
        const selectedValues = Array.from(checkboxes).filter(function(cb) {
          return cb.checked;
        }).map(function(cb) {
          return cb.value;
        });
