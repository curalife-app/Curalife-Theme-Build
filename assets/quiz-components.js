const H="modulepreload",j=function(l){return"/"+l},S={},h=function(e,t,i){let s=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),a=o?.nonce||o?.getAttribute("nonce");s=Promise.allSettled(t.map(n=>{if(n=j(n),n in S)return;S[n]=!0;const g=n.endsWith(".css"),R=g?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${n}"]${R}`))return;const p=document.createElement("link");if(p.rel=g?"stylesheet":H,g||(p.as="script"),p.crossOrigin="",p.href=n,a&&p.setAttribute("nonce",a),document.head.appendChild(p),g)return new Promise((O,Q)=>{p.addEventListener("load",O),p.addEventListener("error",()=>Q(new Error(`Unable to preload CSS for ${n}`)))})}))}function r(o){const a=new Event("vite:preloadError",{cancelable:!0});if(a.payload=o,window.dispatchEvent(a),!a.defaultPrevented)throw o}return s.then(o=>{for(const a of o||[])a.status==="rejected"&&r(a.reason);return e().catch(r)})};class b{constructor(){this.stylesCache=new Map,this.loadingPromises=new Map}async loadStyles(e){if(this.stylesCache.has(e))return this.stylesCache.get(e);if(this.loadingPromises.has(e))return this.loadingPromises.get(e);const t=this.fetchStyles(e);this.loadingPromises.set(e,t);try{const i=await t;return this.stylesCache.set(e,i),this.loadingPromises.delete(e),i}catch(i){throw this.loadingPromises.delete(e),i}}async fetchStyles(e){try{const t=await fetch(e);if(!t.ok)throw new Error(`Failed to load CSS: ${t.status}`);return await t.text()}catch(t){if(console.warn(`Could not load shared styles from ${e}:`,t),e.includes("/assets/quiz.css")){const i=[e.replace("/assets/","/assets/"),"./assets/quiz.css","../assets/quiz.css"];for(const s of i)if(s!==e)try{const r=await fetch(s);if(r.ok)return console.log(`✓ Loaded styles from alternative path: ${s}`),await r.text()}catch{}}return""}}async getQuizStyles(e=null){const t=e||window.QUIZ_CSS_URL||"/assets/quiz.css";return window.QUIZ_CONFIG?.debug&&console.log(`🎨 Loading quiz styles from: ${t}`),this.loadStyles(t)}createStyleElement(e="",t=null){const i=document.createElement("style");return this.getQuizStyles(t).then(s=>{i.textContent=s+`
`+e}).catch(()=>{i.textContent=e}),i.textContent=e,i}setQuizCssUrl(e){window.QUIZ_CSS_URL=e}}const d=new b;class u extends HTMLElement{constructor(){super(),this.config={useShadowDOM:!0,inheritStyles:!0,autoRender:!0},this.isInitialized=!1,this._isComponentConnected=!1,this.config.useShadowDOM?(this.attachShadow({mode:"open"}),this.root=this.shadowRoot):this.root=this,this.handleAttributeChange=this.handleAttributeChange.bind(this),this.handleSlotChange=this.handleSlotChange.bind(this)}connectedCallback(){this._isComponentConnected=!0,this.isInitialized||(this.initialize(),this.isInitialized=!0),this.config.autoRender&&this.render(),this.setupEventListeners(),this.onConnected()}disconnectedCallback(){this._isComponentConnected=!1,this.cleanup(),this.onDisconnected()}attributeChangedCallback(e,t,i){t!==i&&(this.handleAttributeChange(e,t,i),this._isComponentConnected&&this.config.autoRender&&this.render())}initialize(){}render(){throw new Error("render() must be implemented by subclass")}getTemplate(){throw new Error("getTemplate() must be implemented by subclass")}getStyles(){return`
    ...
    `}render(){this.innerHTML=this.getTemplate()}}customElements.get("quiz-calendar-icon")||c.register("quiz-calendar-icon",f);const U=Object.freeze(Object.defineProperty({__proto__:null,default:f},Symbol.toStringTag,{value:"Module"}));class q extends u{constructor(){super(),this.config.useShadowDOM=!1}static get observedAttributes(){return["size","color","stroke-width"]}getTemplate(){const e=this.getAttribute("size")||"24",t=this.getAttribute("color")||"#306E51",i=this.getAttribute("stroke-width")||"2";return`
      <svg
        width="${e}"
        height="${e}"
        viewBox="0 0 24 24"
    ...
    `}render(){this.innerHTML=this.getTemplate()}}customElements.get("quiz-clock-icon")||c.register("quiz-clock-icon",q);const N=Object.freeze(Object.defineProperty({__proto__:null,default:q},Symbol.toStringTag,{value:"Module"}));class v extends u{constructor(){super(),this.config.useShadowDOM=!1}static get observedAttributes(){return["size","color","stroke-width","type"]}getTemplate(){const e=this.getAttribute("size")||"20",t=this.getAttribute("color")||"#306E51",i=this.getAttribute("stroke-width")||"1.5";return(this.getAttribute("type")||"simple")==="circle"?`
        <svg
          width="${e}"
          height="${e}"
          viewBox="0 0 20 20"
    ...
    `}render(){this.innerHTML=this.getTemplate()}}customElements.get("quiz-checkmark-icon")||c.register("quiz-checkmark-icon",v);const F=Object.freeze(Object.defineProperty({__proto__:null,default:v},Symbol.toStringTag,{value:"Module"}));class z extends u{static get observedAttributes(){return["title","sessions-covered","plan-end"]}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}render(){this.getAttribute("title"),this.getAttribute("sessions-covered"),this.getAttribute("plan-end"),this.renderTemplate()}getTemplate(){const e=this.getAttribute("title")||"Here's Your Offer",t=this.getAttribute("sessions-covered")||"5",i=this.getAttribute("plan-end")||"Dec 31, 2025";return`
		...
		`}}customElements.get("quiz-coverage-card")||c.register("quiz-coverage-card",z);const G=Object.freeze(Object.defineProperty({__proto__:null,default:z},Symbol.toStringTag,{value:"Module"}));class y extends u{static get observedAttributes(){return["icon","text","icon-color","icon-size"]}getTemplate(){const e=this.getAttribute("icon")||"checkmark",t=this.getAttribute("text")||"",i=this.getAttribute("icon-color")||"#306E51",s=this.getAttribute("icon-size")||"20";return`
      <div class="quiz-benefit-item">
        <div class="quiz-benefit-icon">
          ${this.getIconHTML(e,i,s)}
        </div>
        <div class="quiz-benefit-text">
          ${this.sanitizeHTML(t)}
        </div>
      </div>
    `}getIconHTML(e,t,i){const s=`width="${i}" height="${i}" role="img"`;switch(e){case"calendar":return`
          <svg ${s} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Calendar">
        ...
        `;case"clock":return`
          <svg ${s} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Clock">
        ...
        `;case"checkmark":default:return`
          <svg ${s} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Checkmark">
...
<path d="M9.99658 10.4166H10.004M13.3262 10.4166H13.3337M6.66699 10.4166H6.67447" stroke="#418865" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
          ...
          <a href="${s}" class="quiz-booking-button">Proceed to booking</a>
        ...
        <div class="quiz-error-content">
          ...
          ${s?`
            <div class="quiz-error-code">
              ...
              <span class="quiz-error-code-value">${this.sanitizeHTML(s)}</span>
            </div>
          `:""}
    ...
    `}render(){this.renderTemplate()}handleAttributeChange(e,t,i){this.isConnected&&this.render()}onConnected(){this.dispatchCustomEvent("quiz-error-display-ready",{type:this.getAttribute("type"),title:this.getAttribute("title"),errorCode:this.getAttribute("error-code")})}setError(e,t,i,s=null,r=!1){this.setAttributes({type:e,title:t,message:i,"error-code":s,"show-details":r})}getError(){return{type:this.getAttribute("type"),title:this.getAttribute("title"),message:this.getAttribute("message"),errorCode:this.getAttribute("error-code"),showDetails:this.getBooleanAttribute("show-details")}}toggleDetails(e=null){const t=this.getBooleanAttribute("show-details"),i=e!==null?e:!t;this.setAttribute("show-details",i)}}customElements.get("quiz-error-display")||c.register("quiz-error-display",w);const W=Object.freeze(Object.defineProperty({__proto__:null,default:w},Symbol.toStringTag,{value:"Module"}));class k extends u{static get observedAttributes(){return["type","title","message","progress","current-step","total-steps","show-spinner"]}getTemplate(){const e=this.getAttribute("type")||"simple",t=this.getAttribute("title")||"Loading...",i=this.getAttribute("message")||"",s=this.getAttribute("progress")||"0",r=this.getAttribute("current-step")||"1",o=this.getAttribute("total-steps")||"1",a=this.getBooleanAttribute("show-spinner",!0);return e==="comprehensive"?this.getComprehensiveTemplate(t,i,s,r,o,a):this.getSimpleTemplate(t,i,a)}getSimpleTemplate(e,t,i){return`
    ...
    `}getComprehensiveTemplate(e,t,i,s,r,o){return`
    ...
    `}render(){this.renderTemplate()}handleAttributeChange(e,t,i){this.isConnected&&(e==="progress"?this.updateProgress(i):this.render())}updateProgress(e){const t=this.querySelector(".quiz-loading-progress-fill"),i=this.querySelector(".quiz-loading-progress-text");if(t&&(t.style.width=`${e}%`),i){const s=this.getAttribute("current-step")||"1",r=this.getAttribute("total-steps")||"1";i.textContent=`${e}% complete (${s}/${r})`}}onConnected(){this.dispatchCustomEvent("quiz-loading-display-ready",{type:this.getAttribute("type"),title:this.getAttribute("title"),progress:this.getAttribute("progress")})}setLoading(e,t,i="",s=0,r=1,o=1){this.setAttributes({type:e,title:t,message:i,progress:s.toString(),"current-step":r.toString(),"total-steps":o.toString()})}setProgress(e,t=null){this.setAttribute("progress",e.toString()),t!==null&&this.setAttribute("current-step",t.toString())}getLoading(){return{type:this.getAttribute("type"),title:this.getAttribute("title"),message:this.getAttribute("message"),progress:parseInt(this.getAttribute("progress")||"0"),currentStep:parseInt(this.getAttribute("current-step")||"1"),totalSteps:parseInt(this.getAttribute("total-steps")||"1")}}toggleSpinner(e=null){const t=this.getBooleanAttribute("show-spinner",!0),i=e!==null?e:!t;this.setAttribute("show-spinner",i)}}customElements.get("quiz-loading-display")||c.register("quiz-loading-display",k);const Y=Object.freeze(Object.defineProperty({__proto__:null,default:k},Symbol.toStringTag,{value:"Module"}));class $ extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.faqData=[]}static get observedAttributes(){return["faq-data"]}attributeChangedCallback(e,t,i){if(e==="faq-data"&&i)try{this.faqData=JSON.parse(i),this.render()}catch(s){console.error("Invalid FAQ data:",s)}}connectedCallback(){this.render()}async render(){const e=await b.getQuizStyles(),t=this.getTemplate(),i=this.getStyles();this.shadowRoot.innerHTML=`
			<style>
				${e}
				${i}
			</style>
		...
		`}attachEventListeners(){this.shadowRoot.querySelectorAll(".quiz-faq-item").forEach(t=>{t.addEventListener("click",()=>{if(t.classList.contains("expanded")){t.classList.remove("expanded"),t.setAttribute("aria-expanded","false");const s=t.querySelector(".quiz-faq-question, .quiz-faq-question-collapsed");s&&(s.className="quiz-faq-question-collapsed")}else{t.classList.add("expanded"),t.setAttribute("aria-expanded","true");const s=t.querySelector(".quiz-faq-question, .quiz-faq-question-collapsed");s&&(s.className="quiz-faq-question")}}),t.addEventListener("keydown",i=>{(i.key==="Enter"||i.key===" ")&&(i.preventDefault(),t.click())})})}setFAQData(e){this.faqData=e,this.render()}}customElements.get("quiz-faq-section")||customElements.define("quiz-faq-section",$);class A extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.selectedPayer="",this.placeholder="Start typing to search for your insurance plan...",this.commonPayers=[],this.questionId="",this.searchTimeout=null}static get observedAttributes(){return["question-id","placeholder","selected-payer","common-payers"]}attributeChangedCallback(e,t,i){switch(e){case"question-id":this.questionId=i||"";break;case"placeholder":this.placeholder=i||"Start typing to search for your insurance plan...";break;case"selected-payer":this.selectedPayer=i||"";break;case"common-payers":try{this.commonPayers=i?JSON.parse(i):[]}catch(s){console.error("Invalid common payers data:",s),this.commonPayers=[]}break}this.shadowRoot.innerHTML&&this.render()}connectedCallback(){this.render()}async render(){const e=await b.getQuizStyles(),t=this.getTemplate(),i=this.getStyles();this.shadowRoot.innerHTML=`
			<style>
				${e}
				${i}
			</style>
		...
		`}attachEventListeners(){const e=this.shadowRoot.querySelector(".quiz-payer-search-input"),t=this.shadowRoot.querySelector(".quiz-payer-search-dropdown"),i=this.shadowRoot.querySelector(".quiz-payer-search-close-btn"),s=this.shadowRoot.querySelector(".quiz-payer-search-container");e.addEventListener("input",r=>{const o=r.target.value.trim();this.handleSearch(o,t)}),e.addEventListener("focus",()=>{e.value.trim()===""&&this.showInitialPayerList(t),this.openDropdown(t,s,e)}),i.addEventListener("click",()=>{this.closeDropdown(t,s,e)}),document.addEventListener("click",r=>{s.contains(r.target)||this.closeDropdown(t,s,e)})}handleSearch(e,t){if(clearTimeout(this.searchTimeout),e.length===0){this.showInitialPayerList(t);return}this.searchTimeout=setTimeout(async()=>{try{const i=await this.searchPayers(e);this.renderSearchResults(i,e,t)}catch(i){console.error("Search error:",i),this.showSearchError(t)}},300)}async searchPayers(e){const t=this.filterCommonPayers(e);if(t.length>0)return t;try{return await this.searchPayersAPI(e)}catch(i){return console.error("API search failed:",i),t}}filterCommonPayers(e){const t=e.toLowerCase();return this.commonPayers.filter(i=>{const s=i.displayName.toLowerCase().includes(t),r=i.aliases?.some(o=>o.toLowerCase().includes(t));return s||r})}async searchPayersAPI(e){return[]}showInitialPayerList(e){const t=e.querySelector(".quiz-payer-search-results");t.innerHTML=this.commonPayers.map(i=>`
		...
		`).join(""),this.attachResultListeners(e)}renderSearchResults(e,t,i){const s=i.querySelector(".quiz-payer-search-results");if(e.length===0){s.innerHTML=`
				<div class="quiz-payer-search-no-results">
					No insurance plans found for "${t}"
				</div>
			`;return}s.innerHTML=e.map(r=>`
			<div class="quiz-payer-search-item" data-payer-id="${r.stediId}" data-payer-name="${r.displayName}">
				<div class="quiz-payer-search-item-name">${this.highlightSearchTerm(r.displayName,t)}</div>
				<div class="quiz-payer-search-item-details">ID: ${r.primaryPayerId}</div>
			</div>
		`).join(""),this.attachResultListeners(i)}attachResultListeners(e){e.querySelectorAll(".quiz-payer-search-item").forEach(i=>{i.addEventListener("click",()=>{const s=i.dataset.payerId,r=i.dataset.payerName;this.selectPayer({stediId:s,displayName:r})})})}selectPayer(e){const t=this.shadowRoot.querySelector(".quiz-payer-search-input"),i=this.shadowRoot.querySelector(".quiz-payer-search-dropdown"),s=this.shadowRoot.querySelector(".quiz-payer-search-container");t.value=e.displayName,this.selectedPayer=e.stediId,this.closeDropdown(i,s,t),this.dispatchEvent(new CustomEvent("payer-selected",{detail:{questionId:this.questionId,payer:e},bubbles:!0}))}openDropdown(e,t,i){e.style.display="block",t.classList.add("dropdown-open"),i.classList.add("dropdown-open")}closeDropdown(e,t,i){e.style.display="none",t.classList.remove("dropdown-open"),i.classList.remove("dropdown-open")}showSearchError(e){const t=e.querySelector(".quiz-payer-search-results");t.innerHTML=`
			<div class="quiz-payer-search-error">
				Unable to search at this time. Please try again.
			</div>
		`}highlightSearchTerm(e,t){if(!t)return e;const i=new RegExp(`(${t})`,"gi");return e.replace(i,'<span class="quiz-payer-search-highlight">$1</span>')}resolvePayerDisplayName(e){if(!e)return"";const t=this.commonPayers.find(i=>i.stediId===e||i.primaryPayerId===e);return t?t.displayName:e}setValue(e){this.selectedPayer=e;const t=this.shadowRoot.querySelector(".quiz-payer-search-input");t&&(t.value=this.resolvePayerDisplayName(e))}getValue(){return this.selectedPayer}}customElements.get("quiz-payer-search")||customElements.define("quiz-payer-search",A);class E extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.resultType="generic",this.resultData={},this.resultUrl=""}static get observedAttributes(){return["result-type","result-data","result-url"]}attributeChangedCallback(e,t,i){switch(e){case"result-type":this.resultType=i||"generic";break;case"result-data":try{this.resultData=i?JSON.parse(i):{}}catch(s){console.error("Invalid result data:",s),this.resultData={}}break;case"result-url":this.resultUrl=i||"";break}this.shadowRoot.innerHTML&&this.render()}connectedCallback(){this.render()}async render(){const e=await b.getQuizStyles(),t=this.getTemplate(),i=this.getStyles();this.shadowRoot.innerHTML=`
			<style>
				${e}
				${i}
			</style>
		...
		`}getErrorTitle(e){return{42:"Insurance Information Issue",43:"Coverage Verification Problem",72:"Plan Details Unavailable",73:"Eligibility Check Failed",75:"Coverage Status Unknown",79:"Verification Timeout"}[e]||"Verification Issue"}attachEventListeners(){this.shadowRoot.querySelectorAll(".result-button").forEach(t=>{t.addEventListener("click",i=>{this.dispatchEvent(new CustomEvent("result-action",{detail:{resultType:this.resultType,action:"button-click",url:t.href},bubbles:!0}))})})}setResultData(e,t,i){this.resultType=e,this.resultData=t,this.resultUrl=i,this.render()}}customElements.get("quiz-result-card")||customElements.define("quiz-result-card",E);class D extends u{static get observedAttributes(){return["step-data","responses","is-last-step","validation-errors"]}getTemplate(){const e=this.getStepData(),t=this.getResponses(),i=this.getAttribute("is-last-step")==="true",s=this.getValidationErrors();if(!e)return'<p class="quiz-error-text">Step configuration error. Please contact support.</p>';const r=i?e.ctaText||"Finish Quiz":e.ctaText||"Continue";return`
			...
			<div class="quiz-form-container">
				...
				<div class="quiz-space-y-6">
					${this.renderFormQuestions(e.questions,t,s)}
				</div>
		...
		`}render(){this.renderTemplate(),this.attachEventListeners()}attachEventListeners(){const e=this.root.querySelector("#quiz-form-next-button");e&&e.addEventListener("click",t=>{t.preventDefault(),this.dispatchEvent(new CustomEvent("form-submit",{detail:{stepData:this.getStepData()},bubbles:!0}))})}renderFormQuestions(e,t,i){if(!e||!Array.isArray(e))return"";let s="",r=0;for(;r<e.length;){const o=this.tryProcessQuestionGroup(e,r,t);o.html?(s+=o.html,r+=o.skip):(s+=this.renderSingleFormQuestion(e[r],t,i),r++)}return s}tryProcessQuestionGroup(e,t,i){const s=e[t],r=a=>i?.find(n=>n.questionId===a.id)||{answer:null},o=[["member-id","group-number"],["first-name","last-name"],["email","phone"]];for(const a of o)if(s.id===a[0]&&e[t+1]?.id===a[1])return{html:this.renderFormFieldPair(s,e[t+1],r(s),r(e[t+1])),skip:2};if(s.type==="date-part"&&s.part==="month"){const[a,n]=[e[t+1],e[t+2]];if(a?.type==="date-part"&&a.part==="day"&&n?.type==="date-part"&&n.part==="year")return{html:this.renderDateGroup(s,a,n,i),skip:3}}return{html:null,skip:0}}renderSingleFormQuestion(e,t,i){const s=t?.find(a=>a.questionId===e.id)||{answer:null},r=i?.some(a=>a.questionId===e.id);return`
			<div class="quiz-question-section ${r?"quiz-field-error":""}">
				<label class="quiz-label" for="question-${e.id}">
					${e.text}${this.renderHelpIcon(e.id)}
				</label>
				${e.helpText?`<p class="quiz-text-sm">${e.helpText}</p>`:""}
				${this.renderQuestionInput(e,s)}
				${r?`<div class="quiz-error-message">${i.find(a=>a.questionId===e.id)?.message||"Invalid input"}</div>`:""}
			</div>
		`}renderFormFieldPair(e,t,i,s){const r=(n,g)=>({input:this.renderQuestionInput(n,g),helpIcon:this.renderHelpIcon(n.id),label:n.text,id:n.id}),[o,a]=[r(e,i),r(t,s)];return`
		...
		`}renderDateGroup(e,t,i,s){const r=s?.find(n=>n.questionId===e.id)||{answer:null},o=s?.find(n=>n.questionId===t.id)||{answer:null},a=s?.find(n=>n.questionId===i.id)||{answer:null};return`
			<div class="quiz-question-section">
				<label class="quiz-label">${e.text}</label>
				<div class="quiz-grid-3">
					${this.renderDatePart(e,r)}
		...
		`:""}renderDatePart(e,t){const i=t?.answer||"",s=this.getDatePartOptions(e.part);return`
			<select id="question-${e.id}" class="quiz-dropdown">
				<option value="">${e.placeholder||e.part}</option>
				${s.map(r=>`
					<option value="${r.value}" ${i===r.value?"selected":""}>
						${r.text}
					</option>
				`).join("")}
			</select>
		`}getDatePartOptions(e){switch(e){case"month":return Array.from({length:12},(i,s)=>({value:String(s+1).padStart(2,"0"),text:new Date(2e3,s).toLocaleString("default",{month:"long"})}));case"day":return Array.from({length:31},(i,s)=>({value:String(s+1).padStart(2,"0"),text:String(s+1)}));case"year":const t=new Date().getFullYear();return Array.from({length:100},(i,s)=>({value:String(t-s),text:String(t-s)}));default:return[]}}renderHelpIcon(e){return`<span class="quiz-help-icon-container">
		...
		`:""}renderFormStep(e,t,i){const s=document.createElement("quiz-form-step");return s.setAttribute("step-data",JSON.stringify(e)),s.setAttribute("responses",JSON.stringify(t)),s.setAttribute("validation-errors",JSON.stringify(i)),this.getAttribute("is-last-step")==="true"&&s.setAttribute("is-last-step","true"),s.outerHTML}renderWizardStep(e,t,i){const s=e.questions[i],r=t?.find(a=>a.questionId===s?.id)||{answer:null};if(!s)return'<p class="quiz-error-text">Question not found. Please try again.</p>';let o="";return e.info?o+=`
				<div class="quiz-divider">
					<h4 class="quiz-heading">${s.text}</h4>
					${s.helpText?`<p class="quiz-text-sm">${s.helpText}</p>`:""}
				</div>
			`:o+=`
				<div class="quiz-question-header">
					<h3 class="quiz-title">${s.text}</h3>
					${s.helpText?`<p class="quiz-text">${s.helpText}</p>`:""}
				</div>
			`,o+=this.renderQuestionByType(s,r),o}renderQuestionByType(e,t){const i=t?.answer||"";switch(e.type){case"multiple-choice":return this.renderMultipleChoice(e,i);case"checkbox":return this.renderCheckbox(e,i);case"dropdown":return this.renderDropdown(e,i);case"text":case"email":case"phone":return this.renderTextInput(e,i);case"textarea":return this.renderTextarea(e,i);case"rating":return this.renderRating(e,i);case"date":return this.renderDateInput(e,i);case"payer-search":return this.renderPayerSearch(e,i);default:return`<p class="quiz-error-text">Unsupported question type: ${e.type}</p>`}}renderMultipleChoice(e,t){return e.options?`
		...
		`:""}renderCheckbox(e,t){if(!e.options)return"";const i=Array.isArray(t)?t:t?[t]:[];return`
			<div class="quiz-checkbox-group">
				${e.options.map(s=>`
					<label class="quiz-option-card ${i.includes(s.value)?"selected":""}">
						<input type="checkbox" name="question-${e.id}" value="${s.value}"
							   ${i.includes(s.value)?"checked":""} class="quiz-checkbox-input">
						<div class="quiz-option-content">
							<span class="quiz-option-text">${s.text}</span>
							${s.description?`<span class="quiz-option-description">${s.description}</span>`:""}
						</div>
					</label>
				`).join("")}
			</div>
		...
		`}renderRating(e,t){const i=e.maxRating||5,s=parseInt(t)||0;return`
			<div class="quiz-rating">
				${Array.from({length:i},(r,o)=>o+1).map(r=>`
					<button type="button" class="quiz-rating-star ${r<=s?"selected":""}"
							data-rating="${r}">
						<svg width="24" height="24" viewBox="0 0 24 24" fill="none">
							<path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"
								  fill="${r<=s?"#fbbf24":"#e5e7eb"}"
		...
		`}render(){this.renderTemplate(),this.attachEventListeners()}attachEventListeners(){this.root.querySelectorAll(".quiz-button").forEach(t=>{t.addEventListener("click",i=>{const s=t.getAttribute("data-action");s&&this.dispatchEvent(new CustomEvent("scheduling-action",{detail:{action:s,target:i.target},bubbles:!0}))})})}renderSuccessResult(e){if(!e)return this.renderGenericSuccess();const t=e.appointment||{},i=e.dietitian||{};return`
		...
		`}getSchedulingData(){try{const e=this.getAttribute("scheduling-data");return e?JSON.parse(e):null}catch(e){return console.error("Error parsing scheduling data:",e),null}}handleAttributeChange(e,t,i){["result-type","scheduling-data","error-message"].includes(e)&&this.render()}}customElements.get("quiz-scheduling-result")||customElements.define("quiz-scheduling-result",M);class T extends u{static get observedAttributes(){return["question-data","selected-value","disabled"]}constructor(){super(),this.questionData=null,this.selectedValue=null,this.isDisabled=!1}initialize(){this.parseAttributes()}parseAttributes(){const e=this.getAttribute("question-data");if(e)try{this.questionData=JSON.parse(e)}catch(t){console.error("Quiz Multiple Choice: Invalid question-data JSON:",t),this.questionData=null}this.selectedValue=this.getAttribute("selected-value")||null,this.isDisabled=this.getBooleanAttribute("disabled",!1)}handleAttributeChange(e,t,i){switch(e){case"question-data":this.parseAttributes();break;case"selected-value":this.selectedValue=i,this.updateSelectedState();break;case"disabled":this.isDisabled=this.getBooleanAttribute("disabled",!1),this.updateDisabledState();break}}getTemplate(){if(!this.questionData||!this.questionData.options)return`
				<div class="quiz-error-container">
					<p class="quiz-error-text">Invalid question configuration</p>
				</div>
			`;const e=this.questionData.options,t=this.questionData.id,i=e.map(s=>`
			<label for="${s.id}" class="quiz-option-card" data-option-id="${s.id}">
				<input
					type="radio"
					id="${s.id}"
					name="question-${t}"
					value="${s.id}"
					class="quiz-sr-only"
					${this.selectedValue===s.id?"checked":""}
					${this.isDisabled?"disabled":""}
				>
				<div class="quiz-option-button ${this.selectedValue===s.id?"selected":""}">
					<div class="quiz-option-text">
						<div class="quiz-option-text-content">${s.text}</div>
					</div>
					${this.selectedValue===s.id?this.getCheckmarkSVG():""}
				</div>
			</label>
		`).join("");return`
		...
		`}render(){this.renderTemplate()}async getStyles(){const e=super.getStyles(),t=await d.getQuizStyles();return`
			${e}
			${t}
		...
		`}setupEventListeners(){this.root.addEventListener("change",this.handleOptionChange.bind(this)),this.root.addEventListener("click",this.handleOptionClick.bind(this))}handleOptionChange(e){if(this.isDisabled)return;const t=e.target;if(t.type==="radio"){const i=t.value;this.selectedValue=i,this.updateSelectedState(),this.dispatchAnswerSelected(i)}}handleOptionClick(e){if(this.isDisabled)return;const t=e.target.closest(".quiz-option-card");if(t){const i=t.getAttribute("data-option-id"),s=t.querySelector("input[type='radio']");s&&!s.checked&&(s.checked=!0,this.selectedValue=i,this.updateSelectedState(),this.dispatchAnswerSelected(i))}}updateSelectedState(){this.root.querySelectorAll(".quiz-option-card").forEach(t=>{const i=t.getAttribute("data-option-id"),s=t.querySelector(".quiz-option-button"),r=t.querySelector("input[type='radio']");if(i===this.selectedValue)s.classList.add("selected"),r.checked=!0,s.querySelector(".quiz-checkmark")||s.querySelector(".quiz-option-text").insertAdjacentHTML("afterend",this.getCheckmarkSVG());else{s.classList.remove("selected"),r.checked=!1;const o=s.querySelector(".quiz-checkmark");o&&o.remove()}})}updateDisabledState(){this.root.querySelectorAll("input[type='radio']").forEach(i=>{i.disabled=this.isDisabled});const t=this.root.querySelector(".quiz-grid-2");t&&(this.isDisabled?t.setAttribute("aria-disabled","true"):t.removeAttribute("aria-disabled"))}dispatchAnswerSelected(e){const t=new CustomEvent("answer-selected",{detail:{questionId:this.questionData?.id,value:e,questionType:"multiple-choice"},bubbles:!0});this.dispatchEvent(t)}getSelectedValue(){return this.selectedValue}setSelectedValue(e){this.selectedValue=e,this.setAttribute("selected-value",e)}setDisabled(e){this.isDisabled=e,e?this.setAttribute("disabled",""):this.removeAttribute("disabled")}getQuestionData(){return this.questionData}setQuestionData(e){this.questionData=e,this.setAttribute("question-data",JSON.stringify(e))}}customElements.get("quiz-multiple-choice")||customElements.define("quiz-multiple-choice",T);class V extends u{static get observedAttributes(){return["question-data","selected-values","disabled","layout"]}constructor(){super(),this.questionData=null,this.selectedValues=[],this.isDisabled=!1,this.layout="cards"}initialize(){this.parseAttributes()}parseAttributes(){const e=this.getAttribute("question-data");if(e)try{this.questionData=JSON.parse(e)}catch(i){console.error("Quiz Checkbox Group: Invalid question-data JSON:",i),this.questionData=null}const t=this.getAttribute("selected-values");if(t)try{this.selectedValues=JSON.parse(t)}catch(i){console.error("Quiz Checkbox Group: Invalid selected-values JSON:",i),this.selectedValues=[]}else this.selectedValues=[];this.isDisabled=this.getBooleanAttribute("disabled",!1),this.layout=this.getAttribute("layout")||"cards",this.questionData?.id==="consent"&&(this.layout="simple")}handleAttributeChange(e,t,i){switch(e){case"question-data":this.parseAttributes();break;case"selected-values":this.parseSelectedValues(),this.updateSelectedState();break;case"disabled":this.isDisabled=this.getBooleanAttribute("disabled",!1),this.updateDisabledState();break;case"layout":this.layout=i||"cards";break}}parseSelectedValues(){const e=this.getAttribute("selected-values");if(e)try{this.selectedValues=JSON.parse(e)}catch(t){console.error("Quiz Checkbox Group: Invalid selected-values JSON:",t),this.selectedValues=[]}else this.selectedValues=[]}getTemplate(){return!this.questionData||!this.questionData.options?`
			...
			<label for="${s.id}" class="quiz-option-card" data-option-id="${s.id}">
				<input
					...
					value="${s.id}"
					class="quiz-sr-only"
					${this.selectedValues.includes(s.id)?"checked":""}
					${this.isDisabled?"disabled":""}
				>
				<div class="quiz-option-button ${this.selectedValues.includes(s.id)?"selected":""}">
					<div class="quiz-option-text">
						<div class="quiz-option-text-content">${s.text}</div>
					</div>
					${this.selectedValues.includes(s.id)?this.getCheckmarkSVG():""}
				</div>
			</label>
		`).join("");return`
			<div class="quiz-grid-2" ${this.isDisabled?'aria-disabled="true"':""}>
		...
		`}getSimpleTemplate(){const e=this.questionData.options,t=this.questionData.id,i=e.map(s=>`
			<div class="quiz-checkbox-container">
				<input
					...
					${this.isDisabled?"disabled":""}
				>
				<label class="quiz-checkbox-label" for="${s.id}">${s.text}</label>
			</div>
		`).join("");return`
		...
		`}setupEventListeners(){this.root.addEventListener("change",this.handleOptionChange.bind(this)),this.root.addEventListener("click",this.handleOptionClick.bind(this))}handleOptionChange(e){if(this.isDisabled)return;const t=e.target;if(t.type==="checkbox"){const i=t.value;t.checked?this.selectedValues.includes(i)||this.selectedValues.push(i):this.selectedValues=this.selectedValues.filter(s=>s!==i),this.updateSelectedState(),this.dispatchAnswerSelected(this.selectedValues)}}handleOptionClick(e){if(!this.isDisabled&&this.layout==="cards"){const t=e.target.closest(".quiz-option-card");if(t){const i=t.getAttribute("data-option-id"),s=t.querySelector("input[type='checkbox']");s&&(s.checked=!s.checked,s.checked?this.selectedValues.includes(i)||this.selectedValues.push(i):this.selectedValues=this.selectedValues.filter(r=>r!==i),this.updateSelectedState(),this.dispatchAnswerSelected(this.selectedValues))}}}updateSelectedState(){this.layout==="cards"?this.updateCardSelectedState():this.updateSimpleSelectedState()}updateCardSelectedState(){this.root.querySelectorAll(".quiz-option-card").forEach(t=>{const i=t.getAttribute("data-option-id"),s=t.querySelector(".quiz-option-button"),r=t.querySelector("input[type='checkbox']");if(this.selectedValues.includes(i))s.classList.add("selected"),r.checked=!0,s.querySelector(".quiz-checkmark")||s.querySelector(".quiz-option-text").insertAdjacentHTML("afterend",this.getCheckmarkSVG());else{s.classList.remove("selected"),r.checked=!1;const o=s.querySelector(".quiz-checkmark");o&&o.remove()}})}updateSimpleSelectedState(){this.root.querySelectorAll("input[type='checkbox']").forEach(t=>{const i=t.value;t.checked=this.selectedValues.includes(i)})}updateDisabledState(){this.root.querySelectorAll("input[type='checkbox']").forEach(i=>{i.disabled=this.isDisabled});const t=this.root.querySelector(".quiz-grid-2, .quiz-space-y-3");t&&(this.isDisabled?t.setAttribute("aria-disabled","true"):t.removeAttribute("aria-disabled"))}dispatchAnswerSelected(e){const t=new CustomEvent("answer-selected",{detail:{questionId:this.questionData?.id,value:e,questionType:"checkbox"},bubbles:!0});this.dispatchEvent(t)}getSelectedValues(){return[...this.selectedValues]}setSelectedValues(e){this.selectedValues=Array.isArray(e)?[...e]:[],this.setAttribute("selected-values",JSON.stringify(this.selectedValues))}setDisabled(e){this.isDisabled=e,e?this.setAttribute("disabled",""):this.removeAttribute("disabled")}getQuestionData(){return this.questionData}setQuestionData(e){this.questionData=e,this.setAttribute("question-data",JSON.stringify(e))}setLayout(e){this.layout=e,this.setAttribute("layout",e)}}customElements.get("quiz-checkbox-group")||customElements.define("quiz-checkbox-group",V);class I extends u{static get observedAttributes(){return["question-data","selected-value","disabled","show-error","error-message"]}constructor(){super(),this.questionData=null,this.selectedValue=null,this.isDisabled=!1,this.showError=!1,this.errorMessage=""}initialize(){this.parseAttributes()}parseAttributes(){const e=this.getAttribute("question-data");if(e)try{this.questionData=JSON.parse(e)}catch(t){console.error("Quiz Dropdown: Invalid question-data JSON:",t),this.questionData=null}this.selectedValue=this.getAttribute("selected-value")||null,this.isDisabled=this.getBooleanAttribute("disabled",!1),this.showError=this.getBooleanAttribute("show-error",!1),this.errorMessage=this.getAttribute("error-message")||""}handleAttributeChange(e,t,i){switch(e){case"question-data":this.parseAttributes();break;case"selected-value":this.selectedValue=i,this.updateSelectedState();break;case"disabled":this.isDisabled=this.getBooleanAttribute("disabled",!1),this.updateDisabledState();break;case"show-error":this.showError=this.getBooleanAttribute("show-error",!1),this.updateErrorState();break;case"error-message":this.errorMessage=i||"",this.updateErrorMessage();break}}getTemplate(){if(!this.questionData)return`
				<div class="quiz-error-container">
					<p class="quiz-error-text">Invalid question configuration</p>
				</div>
			`;const e=this.questionData.options||[],t=this.questionData.id,i=this.questionData.placeholder||"Select an option",s=e.map(r=>`
			<option value="${r.id}" ${this.selectedValue===r.id?"selected":""}>
				${r.text}
			</option>
		`).join("");return`
