...
<path d="M9.99658 10.4166H10.004M13.3262 10.4166H13.3337M6.66699 10.4166H6.67447" stroke="#418865" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    `}render(){this.renderTemplate()}handleAttributeChange(name,oldValue,newValue){this.isConnected&&(name==="progress"?this.updateProgress(newValue):this.render())}updateProgress(progress){const progressFill=this.querySelector(".quiz-loading-progress-fill"),progressText=this.querySelector(".quiz-loading-progress-text");if(progressFill&&(progressFill.style.width=`${progress}%`),progressText){const currentStep=this.getAttribute("current-step")||"1",totalSteps=this.getAttribute("total-steps")||"1";progressText.textContent=`${progress}% complete (${currentStep}/${totalSteps})`}}onConnected(){this.dispatchCustomEvent("quiz-loading-display-ready",{type:this.getAttribute("type"),title:this.getAttribute("title"),progress:this.getAttribute("progress")})}setLoading(type,title,message="",progress=0,currentStep=1,totalSteps=1){this.setAttributes({type,title,message,progress:progress.toString(),"current-step":currentStep.toString(),"total-steps":totalSteps.toString()})}setProgress(progress,currentStep=null){this.setAttribute("progress",progress.toString()),currentStep!==null&&this.setAttribute("current-step",currentStep.toString())}getLoading(){return{type:this.getAttribute("type"),title:this.getAttribute("title"),message:this.getAttribute("message"),progress:parseInt(this.getAttribute("progress")||"0"),currentStep:parseInt(this.getAttribute("current-step")||"1"),totalSteps:parseInt(this.getAttribute("total-steps")||"1")}}toggleSpinner(show=null){const currentShow=this.getBooleanAttribute("show-spinner",!0),newShow=show!==null?show:!currentShow;this.setAttribute("show-spinner",newShow)}};__name(_QuizLoadingDisplay,"QuizLoadingDisplay");let QuizLoadingDisplay=_QuizLoadingDisplay;customElements.get("quiz-loading-display")||quizComponentRegistry.register("quiz-loading-display",QuizLoadingDisplay);const quizLoadingDisplay=Object.freeze(Object.defineProperty({__proto__:null,default:QuizLoadingDisplay},Symbol.toStringTag,{value:"Module"})),_QuizFAQSection=class _QuizFAQSection extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.faqData=[]}static get observedAttributes(){return["faq-data"]}attributeChangedCallback(name,oldValue,newValue){if(name==="faq-data"&&newValue)try{this.faqData=JSON.parse(newValue),this.render()}catch(error){console.error("Invalid FAQ data:",error)}}connectedCallback(){this.render()}async render(){const sharedStylesCSS=await sharedStyles.getQuizStyles(),template=this.getTemplate(),styles=this.getStyles();this.shadowRoot.innerHTML=`
		...
		`}getStyles(){return""}async render(){await this.renderTemplate(),this.setupEventListeners()}setupEventListeners(){const searchInput=this.getInputElement(),dropdown=this.root.querySelector(".quiz-payer-search-dropdown"),closeBtn=this.root.querySelector(".quiz-payer-search-close-btn"),container=this.root.querySelector(".quiz-payer-search-container");if(console.log("🔍 Payer Search - Setting up event listeners:",{hasSearchInput:!!searchInput,hasDropdown:!!dropdown,hasCloseBtn:!!closeBtn,hasContainer:!!container}),!searchInput||!dropdown||!closeBtn||!container){console.error("🔍 Payer Search - Missing required elements for event listeners");return}searchInput.addEventListener("input",e=>{const query=e.target.value.trim();console.log("🔍 Payer Search - Input event:",query),this.handleInputChange(query),this.handleSearch(query,dropdown)}),searchInput.addEventListener("focus",e=>{(this.showError||this.hasVisualError())&&this.clearError(),e.target.value.trim()||this.showInitialPayerList(dropdown),this.openDropdown(dropdown,container,searchInput)}),searchInput.addEventListener("blur",e=>{this.handleInputBlur(),setTimeout(()=>{dropdown.matches(":hover")||this.closeDropdown(dropdown,container,searchInput)},150)}),closeBtn.addEventListener("click",()=>{searchInput.value="",this.selectedPayer="",this.currentValue="",this.closeDropdown(dropdown,container,searchInput),this.clearError()}),dropdown.addEventListener("mousedown",e=>{e.preventDefault()})}handleSearch(query,dropdown){console.log("🔍 Payer Search - handleSearch called:",{query,hasDropdown:!!dropdown}),this.searchTimeout&&clearTimeout(this.searchTimeout);const resultsContainer=dropdown.querySelector(".quiz-payer-search-results");console.log("🔍 Payer Search - Results container found:",!!resultsContainer),resultsContainer&&(resultsContainer.innerHTML='<div class="quiz-payer-search-loading">Searching...</div>'),this.searchTimeout=setTimeout(async()=>{console.log("🔍 Payer Search - Starting search for:",query);try{const results=await this.searchPayers(query);console.log("🔍 Payer Search - Search results:",results?.length||0),this.renderSearchResults(results,query,dropdown)}catch(error){console.error("🔍 Payer Search - Search error:",error),this.showSearchError(dropdown)}},300)}async searchPayers(query){console.log("🔍 Payer Search - searchPayers called:",{query,hasQuizData:!!this.quizData,commonPayersCount:this.commonPayers?.length||0});try{const apiResults=await this.searchPayersAPI(query);if(console.log("🔍 Payer Search - API results:",apiResults?.length||0),apiResults&&apiResults.length>0)return apiResults.map(item=>item.payer)}catch(error){console.warn("🔍 Payer Search - API search failed, falling back to local search:",error)}const localResults=this.filterCommonPayers(query);return console.log("🔍 Payer Search - Local results:",localResults?.length||0),localResults}filterCommonPayers(query){if(console.log("🔍 Payer Search - filterCommonPayers:",{query,commonPayersCount:this.commonPayers?.length||0,firstPayer:this.commonPayers?.[0]}),!query||query.length===0)return this.commonPayers;const queryLower=query.toLowerCase(),filtered=this.commonPayers.filter(payer=>{const nameMatch=payer.displayName?.toLowerCase().includes(queryLower),aliasMatch=payer.aliases?.some(alias=>alias.toLowerCase().includes(queryLower)),match=nameMatch||aliasMatch;return console.log("🔍 Payer Search - Checking payer:",{name:payer.displayName,nameMatch,aliasMatch,match,query:queryLower}),match&&console.log("🔍 Payer Search - Match found:",payer.displayName),match});return console.log("🔍 Payer Search - Filtered results:",filtered.length),filtered}async searchPayersAPI(query){this.quizData||this.getQuizDataFromParent();const apiKey=(this.quizData?.config?.apiConfig||{}).stediApiKey;if(!apiKey||apiKey.trim()==="")return console.warn("Stedi API key not configured or empty, using local search only"),null;const baseUrl="https://healthcare.us.stedi.com/2024-04-01/payers/search",params=new URLSearchParams({query,pageSize:"10",eligibilityCheck:"SUPPORTED"}),url=`${baseUrl}?${params}`,response=await fetch(url,{method:"GET",headers:{Authorization:apiKey,Accept:"application/json"}});if(!response.ok){const errorText=await response.text();throw new Error(`API request failed: ${response.status} - ${errorText}`)}return(await response.json()).items?.map(item=>({payer:{stediId:item.payer.stediId,displayName:item.payer.displayName,primaryPayerId:item.payer.primaryPayerId,aliases:item.payer.aliases||[],score:item.score}}))||[]}showInitialPayerList(dropdown){const results=this.commonPayers;this.renderSearchResults(results,"",dropdown)}renderSearchResults(results,query,dropdown){console.log("🔍 Payer Search - renderSearchResults called:",{resultsCount:results?.length||0,query,hasDropdown:!!dropdown});const resultsContainer=dropdown.querySelector(".quiz-payer-search-results");if(console.log("🔍 Payer Search - Results container in render:",!!resultsContainer),results.length===0)resultsContainer.innerHTML='<div class="quiz-payer-search-no-results">No insurance plans found. Try a different search term.</div>';else{const resultsHTML=results.map((payer,index)=>{const highlightedName=this.highlightSearchTerm(payer.displayName,query),isApiResult=payer.score!==void 0;return`
		...
		`}renderPayerSearch(question,value){const commonPayersAttr=question.commonPayers?`common-payers='${JSON.stringify(question.commonPayers)}'`:"",selectedPayerAttr=value?`selected-payer="${value}"`:"",placeholderAttr=question.placeholder?`placeholder="${question.placeholder}"`:"",quizData=this.getQuizData();console.log("🔧 Step Container - renderPayerSearch quiz data:",{hasQuizData:!!quizData,hasConfig:!!quizData?.config,hasApiConfig:!!quizData?.config?.apiConfig});const quizDataAttr=quizData?`quiz-data='${JSON.stringify({config:quizData.config,commonPayers:quizData.commonPayers})}'`:"",html=`<quiz-payer-search
			question-id="${question.id}"
			${placeholderAttr}
			${commonPayersAttr}
			${selectedPayerAttr}
			${quizDataAttr}
		></quiz-payer-search>`;return console.log("🔧 Step Container - Generated payer search HTML:",html.substring(0,200)+"..."),html}getStepData(){try{const stepDataAttr=this.getAttribute("step-data");return stepDataAttr?JSON.parse(stepDataAttr):null}catch(error){return console.error("Error parsing step data:",error),null}}getResponses(){try{const responsesAttr=this.getAttribute("responses");return responsesAttr?JSON.parse(responsesAttr):[]}catch(error){return console.error("Error parsing responses:",error),[]}}getValidationErrors(){try{const errorsAttr=this.getAttribute("validation-errors");return errorsAttr?JSON.parse(errorsAttr):[]}catch(error){return console.error("Error parsing validation errors:",error),[]}}getQuizData(){try{const quizDataAttr=this.getAttribute("quiz-data");return quizDataAttr?JSON.parse(quizDataAttr):null}catch(error){return console.error("Error parsing quiz data:",error),null}}handleAttributeChange(name,oldValue,newValue){["step-data","responses","current-question-index","is-form-step","validation-errors","quiz-data"].includes(name)&&this.render()}};__name(_QuizStepContainer,"QuizStepContainer");let QuizStepContainer=_QuizStepContainer;customElements.get("quiz-step-container")||customElements.define("quiz-step-container",QuizStepContainer);const _QuizSchedulingResult=class _QuizSchedulingResult extends QuizBaseComponent{static get observedAttributes(){return["result-type","scheduling-data","error-message"]}getTemplate(){const resultType=this.getAttribute("result-type")||"success",schedulingData=this.getSchedulingData(),errorMessage=this.getAttribute("error-message")||"";return resultType==="success"?this.renderSuccessResult(schedulingData):this.renderErrorResult(errorMessage,schedulingData)}getStyles(){return`
			${super.getStyles()}
